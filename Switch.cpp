...
void Switch_Init(void){
  ...
  IOMUX->SECCFG.PINCM[PB1INDEX] = 0x00040081; // block
  IOMUX->SECCFG.PINCM[PA28INDEX] = 0x00040081;  // special move

      IOMUX->SECCFG.PINCM[PB13INDEX] = 0x00040081; //Jump button
    IOMUX->SECCFG.PINCM[PA11INDEX] = 0x00040081; // hit button
  IOMUX->SECCFG.PINCM[PA12INDEX] = 0x00040081; // block
  IOMUX->SECCFG.PINCM[PB19INDEX] = 0x00040081;  // special move

  GPIOA->POLARITY15_0 = (1<<(8<<1)) +  (1<<(13<<1)) + (1<<(11<<1));        // PA8 rising
  GPIOA->POLARITY31_16 = (1<<((28-16)<<1)); // PA28 rising
  GPIOB->POLARITY15_0 = (1<<(13<<1));       // PB1 rising
   GPIOB->POLARITY31_16 = (1<<((19-16)<<1)); // PA28 rising


  GPIOA->CPU_INT.ICLR = (1<<8)|(1<<28)|(1<<11);  // clear bit 8,28
  GPIOB->CPU_INT.ICLR = (1<<4)|(1<<13)|(1<<19); // clear bit 1,4

  GPIOA->CPU_INT.IMASK = (1<<8)|(1<<28)|(1<<11);  // arm PA8 and PA28
  GPIOB->CPU_INT.IMASK = (1<<4)|(1<<13)|(1<<19); // arm PB1 and PB4
